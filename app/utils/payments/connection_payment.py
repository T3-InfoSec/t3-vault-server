...
def pay_for_connection(
    connector_id: bytes,
    connector_type: ConnectorType,
    payment_type: PaymentType = PaymentType.LN,
):
    try:
        ...
        if not isinstance(connector_type, ConnectorType):
            raise ValueError(
                f"Invalid connector type: {connector_type}. Must be one of {[e.value for e in ConnectorType]}"
            )

        print(
            f"Initializing Payment for {connector_type.value} with ID: {fingerprint_hex}"
        )
